/*
IMUduino_AGM.cpp - Originally based on FreeIMU.cpp - A libre and easy to use orientation sensing library for Arduino
...
*/
/*
This is the stripped down version of the FreeIMU library, IMUduino_AGM
*/
...
void IMUduino_AGM::init() {
  
  init(FIMU_ACCGYRO_ADDR, false);
  
}

void IMUduino_AGM::init(bool fastmode) {
...
void IMUduino_AGM::init(int accgyro_addr, bool fastmode) {
...
void IMUduino_AGM::calLoad() {
  if(EEPROM.read(IMUduino_AGM_EEPROM_BASE) == IMUduino_AGM_EEPROM_SIGNATURE) { // check if signature is ok so we have good data
    location = IMUduino_AGM_EEPROM_BASE + 1; // reset location
...
*/
void IMUduino_AGM::getRawValues(int * raw_values) {
