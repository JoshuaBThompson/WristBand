/*
IMUduino_AG.cpp - Originally based on FreeIMU.cpp - A libre and easy to use orientation sensing library for Arduino
...
*/
/*
This is the stripped down version of the FreeIMU library, IMUduino_AG
*/
...
void IMUduino_AG::init() {
  
  init(FIMU_ACCGYRO_ADDR, false);
  
}

void IMUduino_AG::init(bool fastmode) {
...
void IMUduino_AG::init(int accgyro_addr, bool fastmode) {
...
void IMUduino_AG::calLoad() {
  if(EEPROM.read(IMUduino_AG_EEPROM_BASE) == IMUduino_AG_EEPROM_SIGNATURE) { // check if signature is ok so we have good data
    location = IMUduino_AG_EEPROM_BASE + 1; // reset location
...
*/
void IMUduino_AG::getRawValues(int * raw_values) {
