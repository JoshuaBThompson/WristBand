...
#define MAX_AVERAGE_COUNT 20 //max samples to average the rotation angle (may only use a portion of these, which will lead to quicker but less accurate rotation calc)
...
#define MAX_GYRO_TIME_DIFF  20.0*1000 //time (ms) between resetting the pure gyro sensor angle calculation with that of the averaged accel angle calc
...
class RotationFilter
{
  public:
    ...
    void updateIMUAngleWithAccelAngle(void);

    //Variables
    unsigned long timeDiff, prevTime, prevRateGyroTimeDiff, rateGyroTimeDiff;
    float angleDeg;
